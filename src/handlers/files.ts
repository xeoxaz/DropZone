...
export async function handleFilePreview(req: Request, url: URL): Promise<Response> {
  ...
  const mode = url.searchParams.get('mode') || 'multistream';
  ...
  try {
    ...
    const validModes = ['multistream', 'arcpack', 'chunkline'];
    ...
    // Use the decoded path directly (keep Windows backslashes for file system)
