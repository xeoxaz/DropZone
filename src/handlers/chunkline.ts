...
export async function handleChunkLine(req: Request): Promise<Response> {
  try {
    ...
    if (isComplete) {
      ...
      assembledFile = await storage.assembleChunks(filename, totalChunks);
      console.log(`🔗 File assembled: ${assembledFile.savedPath}`);
    }
